...
for i in range(tid,tid+1):
    ...
    compositetransform.AddTransform(transtransform)
    ...
    #call_imgdir=mylist_sorted[i][9]+'../JP2/' #FIXME: trying for input detection instead of fluoro jp2
    call_imgdir='/nfs/mitraweb2/mnt/disk125/main/toXuLi/toRiken/' + patientnumber +'G/JP2/' #for process detection uses
    #call_imgdir='/nfs/mitraweb2/mnt/disk125/main/marmosetRIKEN/NZ/'+ patientnumber + '/' + patientnumber + stype + '/JP2/'
    print call_imgdir
    ...
    if not os.path.exists(outdir+'/'+call_imgfile+ '.jp2') or os.stat(outdir+'/'+call_imgfile+'.jp2').st_size==0:
        ...
        # resample the image
        # debug to see if there is any memory outage
        ...
        #outSlice = sitk.Resample(inSlice, (24000,18000), compositetransform, sitk.sitkLinear, inSlice.GetOrigin(), inSlice.GetSpacing(), (1,0,0,1), PIXVAL)
...
sitk.WriteImage(registeredImg, outputdirectoryname + '/' + patientnumber + '_40_full_fromraw.img')
