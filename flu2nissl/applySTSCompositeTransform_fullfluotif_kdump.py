...
@author: bingxinghuo
Modified for multiple processing
"""
import sys
...
#    mytheta = float(mylist_sorted[11])
...
#            os.system(wdir+'/kduexp_marmoset.sh "%s" "%s" %s %s' % (inputdir, mylist_sorted[10], tempdir, "expanded_"+ mylist_sorted[10] +".tif"))
            
#            inSlice = sitk.ReadImage(tempdir + 'expanded_'+ mylist_sorted[10] +'.tif')
            if os.path.isfile(inputdir + mylist_sorted[10] +'.tif'):
                inSlice = sitk.ReadImage(inputdir + mylist_sorted[10] +'.tif')
    ...
    #            print "writeout"
#                sitk.WriteImage(outSlice, outputdir + mylist_sorted[10] +'.tif')
                sitk.WriteImage(outSlice, tempdir + 'transformed_'+ mylist_sorted[10] +'.tif')
    #            os.remove(tempdir +'expanded_'+mylist_sorted[10] + '.tif')
                
                os.system(wdir+'/kducomp_marmoset.sh %stransformed_%s.tif %s "%s"' % (tempdir,mylist_sorted[10],outputdir,mylist_sorted[10]))
                
                os.remove(tempdir+'transformed_'+mylist_sorted[10]+'.tif')
