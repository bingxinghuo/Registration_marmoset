...
targetmask = sitk.ReadImage(targetmaskfilename,sitk.sitkFloat32)
...
target_ds = sitk.Resample(target,tuple(int(np.round(x/2.0)) for x in target.GetSize()),identityAffine,sitk.sitkLinear,(0,0,0),(0.2,0.2,0.2),(1,0,0,0,1,0,0,0,1),0.0)

# downsample the atlas to 200 um
atlas_ds = sitk.Resample(atlas,tuple(int(np.round(x/2.0)) for x in atlas.GetSize()),identityAffine,sitk.sitkLinear,(0,0,0),(0.2,0.2,0.2),(1,0,0,0,1,0,0,0,1),0.0)

# downsample the target mask to 200um
targetmask_ds = sitk.Resample(targetmask,tuple(int(np.round(x/2.0)) for x in target.GetSize()),identityAffine,sitk.sitkLinear,(0,0,0),(0.2,0.2,0.2),(1,0,0,0,1,0,0,0,1),0.0)
targetmask_ds_np = sitk.GetArrayFromImage(targetmask_ds)
targetmask_ds_np = np.ceil(targetmask_ds_np)
targetmask_ds = sitk.GetImageFromArray(targetmask_ds_np)
targetmask_ds.SetSpacing((0.2,0.2,0.2))
...
learningRate = 0.1
iterations = 200
registration.SetMetricFixedMask(sitk.Cast(targetmask,sitk.sitkUInt8))
...
registration.Execute(sitk.SmoothingRecursiveGaussian(target,0.1),sitk.SmoothingRecursiveGaussian(atlas,0.1) )
