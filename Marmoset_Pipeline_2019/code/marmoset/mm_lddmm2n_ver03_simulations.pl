...
$MM_LDDMM       	= "$BIN_DIRECTORY\/mmlddmm_ver4.0.2";
...
$index=0;
	...
	$index = $index+1;
        $simulations=$ARGV[$index];
	$index = $index+1;
        $imagefraction=$ARGV[$index];
...
#mappings in the new directory
	...
	if($i == 0) {
                # leebc - changed epsilon from 1e-10 to 1e-11
		$parameters3 =  "$simulations $timesteplist[$i] $deltalist[$i] 0.00000000001 $alphalist[$i] 1 1 1000 0 $imagefraction 25"
	}
	else {
		$parameters3 =  "$simulations $timesteplist[$i] $deltalist[$i] 0.00000000001 $alphalist[$i] 1 1 1000 0 $imagefraction 25"
	}
