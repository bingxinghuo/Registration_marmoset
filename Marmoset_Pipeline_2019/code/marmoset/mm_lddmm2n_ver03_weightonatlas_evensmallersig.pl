...
#$MM_LDDMM       	= "/cis/home/can/smallprj2/lddmm/src/lddmm/mm_src7_0110_2012_template/mmlddmm_ver4_template";
$MM_LDDMM               = "$BIN_DIRECTORY\/mmlddmm_ver4_template";
...
$index=0;
	$channel_number = $ARGV[$index];
        for($i=0; $i<$channel_number; $i++) {
                ...
                $j = 4 * $i + 3 ;
                $weightfilelist[$i]=$ARGV[$j];
                $j = 4 * $i + 4 ;
                $sigmalist[$i]=$ARGV[$j];
        }


	$j = 4 * $channel_number + 1 ;
	$output_folder = $ARGV[$j];
	$j = 4 * $channel_number + 2 ;
	$iteration_number = $ARGV[$j];


        for($i=0; $i<$iteration_number; $i++) {
                $j = 4 * $channel_number + 3 + 2*$i ;
                $alphalist[$i]=$ARGV[$j];		
                $j = 4 * $channel_number + 3 + 2*$i + 1 ;
                $timesteplist[$i]=$ARGV[$j];		
                $deltalist[$i] = 1 / $timesteplist[$i];		
print " $alphalist[$i] $timesteplist[$i] \n";
        }
	$index = $j +1;
...
#removing the path from the file lists
	$shortweightimglist[$i]=$weightfilelist[$i];
	chomp $shortweightimglist[$i];
        $shortweightimglist[$i] =~ /^.+\/(.+)/;
        $shortweightimglist[$i] =$1;
#        $shortweightimglist[$i] =~ s/.img/_weight.img/g ;
        $temp1 = ".img";
	$temp2 = "_weight_$j\.img";
        $shortweightimglist[$i] =~ s/$temp1/$temp2/g ;

#creating hdr file lists
        $shortweighthdrlist[$i] =$shortweightimglist[$i];
        $shortweighthdrlist[$i] =~ s/.img/.hdr/g ;
        $weighthdrlist[$i] =$weightfilelist[$i];
#        $weighthdrlist[$i] =~ s/.img/.hdr/g ;	
        $temp1 = ".img";
        $temp2 = ".hdr";
        $weighthdrlist[$i] =~ s/$temp1/$temp2/g ;
	...
	$sim1 = `cp $weightfilelist[$i] $output_folder/imgfiles/$shortweightimglist[$i]`;
	$sim1 = `cp $weighthdrlist[$i] $output_folder/imgfiles/$shortweighthdrlist[$i]`;
	...
	$sim1 = `cp $weightfilelist[$i] $output_folder/$shortweightimglist[$i]`;
	$sim1 = `cp $weighthdrlist[$i] $output_folder/$shortweighthdrlist[$i]`;
}
...
if($even_size>0){
	for($i=0; $i<$channel_number; $i++) {
		$temp1 = "$output_folder/imgfiles/$shortweightimglist[$i]";
...
#        $currentshorttemplatehdrlist[$i] =~ s/_template.hdr/_curtemplate.hdr/g ;
	...
	$currentshortweightimglist[$i] = $shortweightimglist[$i];
        $temp1 = "_weight_$j\.img";
        $temp2 = "_curweight_$j\.img";
        $currentshortweightimglist[$i] =~ s/$temp1/$temp2/g ;
#        $currentshortweightimglist[$i] =~ s/_weight.img/_curweight.img/g ;

	$currentshortweighthdrlist[$i] = $shortweighthdrlist[$i];
        $temp1 = "_weight_$j\.hdr";
        $temp2 = "_curweight_$j\.hdr";
        $currentshortweighthdrlist[$i] =~ s/$temp1/$temp2/g ;
#        $currentshortweighthdrlist[$i] =~ s/_weight.hdr/_curweight.hdr/g ;

	$temp1 = "$output_folder/imgfiles/$shortweightimglist[$i]";
	$sim1 = `cp $temp1 $output_folder/imgfiles/$currentshortweightimglist[$i]`;
	$temp1 = "$output_folder/imgfiles/$shortweighthdrlist[$i]";
	$sim1 = `cp $temp1 $output_folder/imgfiles/$currentshortweighthdrlist[$i]`;
	$temp1 = "$output_folder/imgfiles/$shorttemplateimglist[$i]";
...
for($j=0; $j<$channel_number; $j++) {
	...
	$currentshortweightimg[$j] = "./imgfiles/$currentshortweightimglist[$j]";
}
...
#mappings in the new directory
	...
	for($j=0; $j<$channel_number; $j++) {
		if($j == 0) {
			$parameters2 = "$parameters2$currentshorttemplateimg[$j] $currentshorttargetimg[$j] $currentshortweightimg[$j] $sigmalist[$j]";	
		}
		else {
			$parameters2 = "$parameters2 $currentshorttemplateimg[$j] $currentshorttargetimg[$j] $currentshortweightimg[$j] $sigmalist[$j]";	
		}
...
for($j=0; $j<$channel_number; $j++) {
	$temp1 = "./$shortweightimglist[$j]";
	$temp2 = "./imgfiles/$shortnewweightimglist[$j]";
	$sim1 = `mv $temp1 $temp2`;
	...
	$temp1 = "./$shortweighthdrlist[$j]";
	$temp2 = "./imgfiles/$shortnewweighthdrlist[$j]";
	$sim1 = `mv $temp1 $temp2`;
...
#		$shortnewtargetimglist[$j]  =~ s/_target.img/_deftarget\_00$k.img/g ;
		...
		$shortnewweightimglist[$j] = $shortweightimglist[$j];
	        $temp1 = "_weight_$jj\.img";
        	$temp2 = "_weight_$jj\_$k\.img";
        	$shortnewweightimglist[$j] =~ s/$temp1/$temp2/g ;
#		$shortnewweightimglist[$j]  =~ s/_weight.img/_defweight\_00$k.img/g ;

#		print "$shortnewtemplateimglist[$j] \n";
#		print "$shortnewtargetimglist[$j] \n";

                $parameters3 = "./imgfiles/$shortweightimglist[$j] ./$Hfile ./imgfiles/$shortnewweightimglist[$j] $interpolationtype";
#                $parameters3 = "./$shortweightimglist[$j] ./$Hfile ./imgfiles/$shortnewweightimglist[$j] $interpolationtype";
                $sim1 = `$IMG_TRANSFORM $parameters3`;
#print "$sim1\n";
