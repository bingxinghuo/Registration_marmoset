...
x = (0:downsamplefactor:nx(1)-1)*dx(1);
y = (0:downsamplefactor:nx(2)-1)*dx(2);
z = (0:downsamplefactor:nx(3)-1)*dx(3);
x = x - mean(x);
y = y - mean(y);
z = z - mean(z);
...
%     % using these for 40um mouse brain to allen atlas
    ...
    %epsilonxy = 0.000002;%0.000012
    %epsilontheta = 0.0000002;
    epsilonxy = 0.000005;
    epsilontheta = 0.0000004;
    
    % maybe increase this epsilonxy

    minepsilonxy = 1e-20;
    minepsilontheta = 2*10^-24;
    sigmaxy = 0.7;
    sigmatheta = pi/180*2;
    sigma_atlas = 0.9;%sigma_atlas = 1;
    %the sigma of atlas is the ratio of how similar the target is to atlas
    %and how similar it is to its neighbor. 1/sigma_atlas, so the smaller
    %sigma the target will be more like the atals
    sigma_target = 1;
    % maybe reduce sigma_target for the end sections (like the cerebellum)
    sigma_target_radius = 17;
    min_sigma_target = 1;
