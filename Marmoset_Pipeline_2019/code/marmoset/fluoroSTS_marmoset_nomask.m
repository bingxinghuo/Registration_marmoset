function fluoroSTS(nisslfilename, fluorofilename, outputdirectoryname, patientnumber)
...
%for i = 1:size(fluoro.img,2)
%    myslice = squeeze(fluoro.img(:,i,:));
%    inflectionpoint = mean(mean(myslice(find(myslice>0.01)))) + std(myslice(find(myslice>0.01)))*0.7;
%    myslice(find(myslice<inflectionpoint)) = 0;
%    fluoro.img(:,i,:) = myslice;
%end
...
fluoro = avw_img_read([outputdirectoryname '/' patientnumber '_fluoro_pre'],0);
