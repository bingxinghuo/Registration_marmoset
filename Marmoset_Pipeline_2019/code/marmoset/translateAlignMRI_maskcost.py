...
costmaskfilename = sys.argv[3]
outputmrifilename = sys.argv[4]
outputtransformfilename = sys.argv[5]
...
costmask = sitk.ReadImage(costmaskfilename,sitk.sitkUInt8)
...
registration.SetMetricFixedMask(costmask)
...
registration.Execute(target_ds,mri)
euler3d = list(identityAffine.GetMatrix()) + list(transtransform.GetOffset())

# resample image
#mriOut = ndreg3D.imgApplyAffine(mri, euler3d, useNearest=False, size=target.GetSize())
mriOut = sitk.Resample(mri,target.GetSize(),transtransform,sitk.sitkLinear,(0,0,0),target.GetSpacing(),(1,0,0,0,1,0,0,0,1),0.0)

#sitk.WriteImage(mriOut,'/cis/home/leebc/Projects/Mouse_Histology/data/marmoset/test/MRI_rigid.img')
sitk.WriteImage(mriOut,outputmrifilename)

# save the transform
#mytransformfile = open("/cis/home/leebc/Projects/Mouse_Histology/data/marmoset/test/M920_globalaffinetrans.txt", "w")
mytransformfile = open(outputtransformfilename, "w")
for item in euler3d:
