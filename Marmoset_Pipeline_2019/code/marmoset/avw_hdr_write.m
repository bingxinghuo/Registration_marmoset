...
% $Revision: 1.2 $ $Date: 2006/06/07 00:07:44 $
...
function data_history(fid,hist)
    ...
    % now that we have char variables, remove all the non-significant
    % white space at the front and back of the strings
    
    descrip     = strtrim(descrip);
    aux_file    = strtrim(aux_file);
    originator  = strtrim(originator);
    generated   = strtrim(generated);
    scannum     = strtrim(scannum);
    patient_id  = strtrim(patient_id);
    exp_date    = strtrim(exp_date);
    exp_time    = strtrim(exp_time);
    hist_un0    = strtrim(hist_un0);
    
    % Now insert null-char, ie double(0), as required
    
    if length(descrip) < 80,
      paddingN = 80-length(descrip);
      padding = char(zeros(1,paddingN));
    ...
    % check for undefined values outside the ASCII char set
    
    if find(double(descrip)>127),
      indexStrangeChar = find(double(descrip)>127);
      descrip(indexStrangeChar) = ' ';
    end
    if find(double(aux_file)>127),
      indexStrangeChar = find(double(aux_file)>127);
      aux_file(indexStrangeChar) = ' ';
    end
    if find(double(originator)>127),
      indexStrangeChar = find(double(originator)>127);
      originator(indexStrangeChar) = ' ';
    end
    if find(double(generated)>127),
      indexStrangeChar = find(double(generated)>127);
      generated(indexStrangeChar) = ' ';
    end
    if find(double(scannum)>127),
      indexStrangeChar = find(double(scannum)>127);
      scannum(indexStrangeChar) = ' ';
    end
    if find(double(patient_id)>127),
      indexStrangeChar = find(double(patient_id)>127);
      patient_id(indexStrangeChar) = ' ';
    end
    if find(double(exp_date)>127),
      indexStrangeChar = find(double(exp_date)>127);
      exp_date(indexStrangeChar) = ' ';
    end
    if find(double(exp_time)>127),
      indexStrangeChar = find(double(exp_time)>127);
      exp_time(indexStrangeChar) = ' ';
    end
    if find(double(hist_un0)>127),
      indexStrangeChar = find(double(hist_un0)>127);
