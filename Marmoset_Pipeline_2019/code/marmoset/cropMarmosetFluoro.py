...
inflectionpoint = int(np.round(np.mean(imgNP[np.where(imgNP > 1)])))
#mymask = ndreg3D.imgMakeMask(img,threshold=inflectionpoint)
mymask = np.ones(imgNP.shape)
mymask[np.where(imgNP<inflectionpoint)] = 0
mymaskimg = sitk.GetImageFromArray(mymask)
mymaskimg.SetSpacing(img.GetSpacing())
...
tmpMask = opener.Execute(mymaskimg)
