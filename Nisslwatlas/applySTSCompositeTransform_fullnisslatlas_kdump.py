...
def mp_transformer(params): 
    ...
    cropmatrix=params[1]
    content5line=params[2]
    mylist2=params[3]
    content4line=params[4]
    tifpixelsize=params[5]
    originalpixelsize=params[6]
    outxsize=params[7]
    outysize=params[8]
    inputdir=params[9]
    outputdir=params[10]
    ...
    euler2dobj2.SetCenter([x*0.08 for x in rotcenter2])
...
if __name__ == '__main__':
    ...
    # first alignment
    transform3matrix = sys.argv[6]
    #transform3file = sys.argv[5]
    #sts
    transform4matrix = sys.argv[7]
    transform4file = sys.argv[8]
    #finalalign
    transform5matrix = sys.argv[9]
    
    # pre-calculated annotation stack
    annostack=sitk.ReadImage(sys.argv[10])
    regxsize=annostack.GetDepth()
    regysize=annostack.GetWidth()

    originalpixelsize = float(sys.argv[11])
...
#    for i in range(tid1+1-tid0):
        allparams[i]=[[] for x in range(11)]
        
        allparams[i][0]=mylist_sorted[i+tid0]
        allparams[i][1]=cropmatrix
        allparams[i][2]=content5line
        allparams[i][3]=mylist2[i+tid0]
        allparams[i][4]=content4line
        allparams[i][5]=tifpixelsize
        allparams[i][6]=originalpixelsize
        allparams[i][7]=outxsize
        allparams[i][8]=outysize
        allparams[i][9]=inputdir
        allparams[i][10]=outputdir
